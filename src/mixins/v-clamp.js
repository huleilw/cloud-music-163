/**
 * clapm.js 超出部分显示三个点（...）
 * github: https://github.com/xavi160/Clamp.js
 */
const clamp = (element, options) => {
    options = options || {}
    let win = window
    let opt = {
        clamp: options.value ? options.value.clamp : options.clamp || 2,
        useNativeClamp:
...
export default {
    directives: {
        clamp: {
            bind(el, binding) {
                clamp(el, Object.assign({ clamp: 2 }, binding))
            },
            update(el, binding) {
                clamp(el, Object.assign({ clamp: 2 }, binding))
            }
